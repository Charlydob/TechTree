...
export default function App(){
 ...
 const [folderCreate,setFolderCreate]=useState<FolderCreateIntent>();
 ...
 const createFolder=(name:string,parentId?:string)=>{const cleanName=name.trim();if(!cleanName)return;const parentPath=parentId?folderPaths.get(parentId):'';const path=parentPath?`${parentPath}/${cleanName}`:cleanName;if([...folderPaths.values()].some(value=>normalize(value)===normalize(path)))return;setFolders(items=>[...items,{id:uniqueFolderId(cleanName),name:cleanName,parentId,createdAt:new Date().toISOString()}]);setFolderFilter(path);setFolderSheetOpen(false);setFolderCreate(undefined);if(parentId)setOpenFolders(ids=>new Set(ids).add(parentId))};
 ...
 return <div className={dark?'app dark':'app'}>
  <header>
   ...
   <div className="header-actions">
    ...
    <details className="mobile-header-menu"><summary aria-label={t.menu}><MoreVertical size={18}/></summary><div>
     <label className="language-select" title={t.language}><span>{t.language}</span><select value={lang} onChange={event=>setLang(event.target.value as Language)}><option value="es">{t.spanish}</option><option value="en">{t.english}</option></select></label>
     <button title={t.toggleTheme} onClick={()=>setDark(value=>!value)} aria-label={t.toggleTheme}>{dark?<Sun size={18}/>:<Moon size={18}/>} {t.toggleTheme}</button>
    </div></details>
   </div>
  </header>
  ...
  {mode==='library'&&<main className="home">
   ...
   <section className="dashboard-grid" id="library">
    <button className="folder-sheet-trigger" onClick={()=>setFolderSheetOpen(true)}><PanelTopOpen size={16}/> {t.folders}: {folderFilter===undefined?t.allFolders:folderLabel(folderFilter,t)}</button>
    <aside className={folderSheetOpen?'folder-rail open':'folder-rail'}>
     <button className="folder-sheet-close" onClick={()=>setFolderSheetOpen(false)}>{t.close}</button>
     <div className="rail-head"><b>{t.folders}</b><button onClick={()=>{setFolderFilter(undefined);setFolderSheetOpen(false)}} className={folderFilter===undefined?'active':''}>{t.allFolders}</button></div>
     <div className="folder-create"><button className="primary" onClick={()=>setFolderCreate({})}>{t.newFolder}</button></div>
     <div className="folder-list">
      <FolderTree folders={folders} books={books} selected={folderFilter} open={openFolders} paths={folderPaths} t={t} setSelected={path=>{setFolderFilter(path);setFolderSheetOpen(false)}} toggle={id=>setOpenFolders(ids=>{const next=new Set(ids);if(next.has(id))next.delete(id);else next.add(id);return next})} addSubfolder={parentId=>setFolderCreate({parentId})} rename={renameFolder} move={moveFolder} remove={folder=>setDeleteFolder({folder,count:books.filter(book=>{const path=folderPaths.get(folder.id)??folder.name;const current=folderOf(book);return current===path||current.startsWith(`${path}/`)}).length})}/>
      {books.some(item=>folderOf(item)===uncategorized)&&<button className={folderFilter===uncategorized?'folder-root active':'folder-root'} onClick={()=>{setFolderFilter(uncategorized);setFolderSheetOpen(false)}}>{t.noFolder}</button>}
     </div>
    </aside>
  ...
  {folderCreate&&<FolderCreateModal intent={folderCreate} folders={folders} folderPaths={folderPaths} close={()=>setFolderCreate(undefined)} create={createFolder} t={t}/>}
...
function FlowEditor({initial,startAt,onSave,onExit,lang,t}:{initial:Runbook;startAt?:string;onSave:(book:Runbook)=>void;onExit:()=>void;lang:Language;t:Record<string,string>}){
 ...
 const [active,setActive]=useState(startAt&&getNode(initial,startAt)?startAt:initial.startNode);
 const [selectedEdge,setSelectedEdge]=useState<SelectedEdge>();
 const [openAddKey,setOpenAddKey]=useState<string>();
 const [nodePicker,setNodePicker]=useState<NodePickerIntent>();
 ...
 const openOutcomeEditor=(sourceId:string,outcome?:Outcome,index?:number)=>setOutcomeIntent({sourceId,outcome,index});
 const addOutcome=(sourceId:string)=>openOutcomeEditor(sourceId);
 const saveOutcome=(sourceId:string,outcome:Outcome,index?:number)=>mutate(draft=>{const source=getNode(draft,sourceId);if(!source)return;if(index!=null&&source.outcomes?.[index])source.outcomes[index]=outcome;else source.outcomes=[...(source.outcomes??[]),outcome]});
 ...
 return <main className="editor visual-editor">
  <div className="editor-head">
   ...
   <div className="editor-toolbar" aria-label={t.editorTitle}><button disabled={!past.length} onClick={undo} title={t.undo} aria-label={t.undo}>&lt;-</button><button disabled={!future.length} onClick={redo} title={t.redo} aria-label={t.redo}>-&gt;</button><button onClick={relayout} title={t.autoLayout}>Auto</button><button onClick={()=>flow.fitView({padding:.16})} title={t.fitView}><Maximize size={16}/> Fit</button><button onClick={()=>activeNode&&flow.setCenter(activeNode.ui?.x??0,activeNode.ui?.y??0,{zoom:1,duration:250})} title={t.center}>Center</button><button onClick={resetLayout} title={t.reset}><RotateCcw size={16}/> Reset</button><button onClick={()=>setShowJson(value=>!value)} title="JSON"><FileJson size={16}/> JSON</button><button onClick={()=>download(book)} title={t.exportJson}><Download size={16}/> JSON</button><button className="primary" disabled={!validation.valid} onClick={save} title={t.save}><Save size={16}/> {t.save} {dirty?'*':''}</button></div>
  </div>
  ...
  <div className="global-quick"><b>{t.quickBuild}</b><span>{t.globalNodeHelp}</span><button className="primary mobile-node-picker-button" onClick={()=>setNodePicker({mode:'loose'})}><Plus size={14}/> {t.addNode}</button><div className="quick-node-buttons">{nodeTypesList.map(type=><button key={type} onClick={()=>addLooseNode(type)}><Plus size={14}/> {nodeTypeLabel(type,t)}</button>)}</div></div>
  <div className="visual-shell">
   ...
   {activeNode&&<section className="inspector node-sheet">
    ...
    <div className="quick-builder node-local-actions"><b>{t.nodeOutcomeHelp}</b><button className="primary" onClick={()=>setNodePicker({mode:'linked',sourceId:activeNode.id})}><Plus size={14}/> {t.addNode}</button><button onClick={()=>addOutcome(activeNode.id)}><GitBranchPlus size={14}/> {t.addBranch}</button><div className="quick-node-buttons">{nodeTypesList.map(type=><button key={type} onClick={()=>addLinkedNode(activeNode.id,type)}><Plus size={14}/> {nodeTypeLabel(type,t)}</button>)}</div></div>
    ...
    <details className="node-section" open><summary>{t.basic}</summary><NodeForm activeNode={activeNode} book={book} lang={lang} t={t} mutate={mutate} setText={setText}/></details>
    <details className="node-section" open={activeNode.type==='troubleshooting'}><summary>{t.diagnostics}</summary>{activeNode.type==='troubleshooting'?<TroubleshootingFields node={activeNode} lang={lang} t={t} mutate={mutate} active={activeNode.id} setText={setText}/>:<p className="muted-inline">{t.none}</p>}</details>
    <details className="node-section" open><summary>{t.flow} <span className="help" title={t.branchHelp}><CircleHelp size={15}/></span></summary>
     <div className="branch-list">{activeNode.outcomes?.map((outcome,index)=><OutcomeRow key={outcome.id+index} outcome={outcome} nodes={book.nodes} lang={lang} t={t} change={next=>mutate(draft=>{getNode(draft,activeNode.id)!.outcomes![index]=next})} edit={()=>openOutcomeEditor(activeNode.id,outcome,index)} remove={()=>mutate(draft=>{getNode(draft,activeNode.id)!.outcomes!.splice(index,1)})} createNode={()=>setNodePicker({mode:'linked',sourceId:activeNode.id,outcomeId:outcome.id})}/>)}</div>
     <button onClick={()=>addOutcome(activeNode.id)}><GitBranchPlus size={16}/> {t.addBranch}</button>
     <label>{t.defaultNext}<select value={activeNode.nextNode??''} onChange={event=>mutate(draft=>{getNode(draft,activeNode.id)!.nextNode=event.target.value||undefined})}><option value="">{t.none}</option>{book.nodes.filter(item=>item.id!==activeNode.id).map(item=><option value={item.id} key={item.id}>{localize(item.title,lang)}</option>)}</select></label>
    </details>
    <details className="node-section"><summary>{t.multimedia}</summary><MediaEditor media={activeNode.media??[]} lang={lang} t={t} change={media=>mutate(draft=>{getNode(draft,activeNode.id)!.media=media})}/></details>
    <details className="node-section"><summary>{t.advanced}</summary><div className={validation.valid?'valid':'errors'}><b>{validation.valid?'OK '+t.valid:t.issues}</b>{[...validation.errors,...validation.warnings].map((error,index)=><p key={index}>{humanValidationError(error,t)}</p>)}</div></details>
   </section>}
  </div>
  {showJson&&<pre className="json-preview">{JSON.stringify(book,null,2)}</pre>}
  {nodePicker&&<NodeTypePickerModal intent={nodePicker} close={()=>setNodePicker(undefined)} choose={type=>{if(nodePicker.mode==='loose')addLooseNode(type);else addLinkedNode(nodePicker.sourceId,type,nodePicker.outcomeId);setNodePicker(undefined)}} t={t}/>}
  {outcomeIntent&&<OutcomeModal intent={outcomeIntent} nodes={book.nodes} lang={lang} t={t} close={()=>setOutcomeIntent(undefined)} save={(outcome,index)=>{saveOutcome(outcomeIntent.sourceId,outcome,index);setOutcomeIntent(undefined)}}/>}
 </main>;
}
...
function OutcomeRow({outcome,nodes,lang,t,change,edit,remove,createNode}:{outcome:Outcome;nodes:RunbookNode[];lang:Language;t:Record<string,string>;change:(outcome:Outcome)=>void;edit:()=>void;remove:()=>void;createNode:()=>void}){
 const target=nodes.find(node=>node.id===outcome.nextNode);
 return <div className="outcome-edit"><button className="outcome-summary" onClick={edit}><span>{localize(outcome.label,lang)||t.addBranch}</span><small>-&gt; {target?localize(target.title,lang):t.end}</small></button><details className="overflow-menu"><summary aria-label={t.menu}>...</summary><div><button onClick={edit}>{t.editBranch}</button><button onClick={createNode}>{t.createNextNode}</button><label>{t.changeTarget}<select value={outcome.nextNode??''} onChange={event=>change({...outcome,nextNode:event.target.value||undefined})}><option value="">{t.end}</option>{nodes.map(node=><option key={node.id} value={node.id}>{localize(node.title,lang)}</option>)}</select></label><button className="danger" onClick={remove}>{t.delete}</button></div></details></div>;
}
...
function NodeTypePickerModal({intent,close,choose,t}:{intent:NodePickerIntent;close:()=>void;choose:(type:NodeType)=>void;t:Record<string,string>}){
 return <div className="modal" role="dialog" aria-modal="true"><section className="compact-modal node-type-modal"><button className="close" onClick={close}>x</button><p className="eyebrow">{intent.mode==='loose'?t.quickBuild:t.addNode}</p><h2>{t.addNodePrompt}</h2><div className="node-type-grid">{nodeTypesList.map(type=><button key={type} onClick={()=>choose(type)}><Plus size={15}/> {nodeTypeLabel(type,t)}</button>)}</div></section></div>;
}

function OutcomeModal({intent,nodes,lang,t,close,save}:{intent:OutcomeIntent;nodes:RunbookNode[];lang:Language;t:Record<string,string>;close:()=>void;save:(outcome:Outcome,index?:number)=>void}){
 const [label,setLabel]=useState(localize(intent.outcome?.label,lang));
 const [nextNode,setNextNode]=useState(intent.outcome?.nextNode??'');
 const apply=()=>save({id:intent.outcome?.id??`outcome-${Date.now()}`,label:localized(label.trim()||t.addBranch,lang),nextNode:nextNode||undefined},intent.index);
 return <div className="modal" role="dialog" aria-modal="true"><section className="compact-modal"><button className="close" onClick={close}>x</button><p className="eyebrow">{t.flow}</p><h2>{intent.outcome?t.editBranch:t.addBranch}</h2><div className="form-grid"><label>{t.title}<input value={label} onChange={event=>setLabel(event.target.value)} autoFocus/></label><label>{t.changeTarget}<select value={nextNode} onChange={event=>setNextNode(event.target.value)}><option value="">{t.end}</option>{nodes.map(node=><option key={node.id} value={node.id}>{localize(node.title,lang)}</option>)}</select></label></div><div className="modal-actions"><button onClick={close}>{t.cancel}</button><button className="primary" onClick={apply}>{t.save}</button></div></section></div>;
}

function humanValidationError(error:string,t:Record<string,string>){
 if(error.includes('/media/')&&error.includes('/url')&&error.includes('fewer than 1 characters'))return t.image==='Imagen'?'Esta imagen necesita una URL.':'This media item needs a URL.';
 if(error.includes('/media/')&&error.includes('/url')&&error.includes('invalid URL'))return t.image==='Imagen'?'La URL del multimedia no es valida.':'The media URL is not valid.';
 if(error.includes('/title')&&error.includes('at least one language'))return t.image==='Imagen'?'El nodo necesita un titulo.':'The node needs a title.';
 return error;
}
...
function MobileCards({book,active,setActive,lang,t,addAfter,addOutcome,insertBefore,remove}:{book:Runbook;active:string;setActive:(id:string)=>void;lang:Language;t:Record<string,string>;addAfter:(sourceId:string,type:NodeType,outcomeId?:string)=>void;addOutcome:(sourceId:string)=>void;insertBefore:()=>void;remove:()=>void}){
 const links=flowLinks(book);
 const ordered=[getNode(book,book.startNode),...book.nodes.filter(node=>node.id!==book.startNode)].filter(Boolean) as RunbookNode[];
 return <section className="cards-surface">{ordered.map(node=>{
  const outgoing=links.filter(link=>link.source===node.id);
  const solutionCount=outgoing.filter(link=>getNode(book,link.target)?.type==='solution').length;
  return <article className={active===node.id?'mobile-node-card active':'mobile-node-card'} key={node.id} onClick={()=>setActive(node.id)}>
   <div className="mobile-node-head"><span>{node.id===book.startNode?'START':node.type}</span><button aria-label={t.nodeOpen} onClick={event=>{event.stopPropagation();setActive(node.id)}}>&gt;</button></div>
   <h3>{localize(node.title,lang)||t.node}</h3>
   <p>{outgoing.length} {t.outcomes.toLowerCase()} / {solutionCount} {t.markAsSolution.toLowerCase()}</p>
   <div className="mobile-branch-list">{outgoing.slice(0,3).map(link=><button key={`${link.source}-${link.outcomeId??'next'}-${link.target}`} onClick={event=>{event.stopPropagation();setActive(link.target)}}>{link.label||t.continue} -&gt; {localize(getNode(book,link.target)?.title,lang)||link.target}</button>)}</div>
   {active===node.id&&<div className="mobile-card-actions"><button onClick={event=>{event.stopPropagation();addAfter(node.id,'action')}}>{t.addNext}</button><button onClick={event=>{event.stopPropagation();addOutcome(node.id)}}>{t.addBranch}</button><button onClick={event=>{event.stopPropagation();insertBefore()}}>{t.insertBefore}</button><button className="danger" onClick={event=>{event.stopPropagation();remove()}}>{t.delete}</button></div>}
  </article>;
 })}</section>;
}

function FolderCreateModal({intent,folders,folderPaths,close,create,t}:{intent:FolderCreateIntent;folders:FolderItem[];folderPaths:Map<string,string>;close:()=>void;create:(name:string,parentId?:string)=>void;t:Record<string,string>}){
 const [name,setName]=useState('');
 const [parentId,setParentId]=useState(intent.parentId??'');
 return <div className="modal" role="dialog" aria-modal="true"><section className="compact-modal"><button className="close" onClick={close}>x</button><p className="eyebrow">{t.folders}</p><h2>{t.createFolderTitle}</h2><div className="form-grid"><label>{t.folderName}<input value={name} onChange={event=>setName(event.target.value)} autoFocus/></label><label>{t.location}<select value={parentId} onChange={event=>setParentId(event.target.value)}><option value="">{t.allFolders}</option>{folders.map(folder=><option key={folder.id} value={folder.id}>{folderPaths.get(folder.id)??folder.name}</option>)}</select></label></div><div className="modal-actions"><button onClick={close}>{t.cancel}</button><button className="primary" disabled={!name.trim()} onClick={()=>create(name,parentId||undefined)}>{t.create}</button></div></section></div>;
}
